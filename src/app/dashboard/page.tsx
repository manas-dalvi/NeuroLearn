...
import { handleResumeSession } from "@/lib/navigation";
...
export default function DashboardPage() {
  ...
  const setStoreSessionId = useDashboardStore((state) => state.setCurrentSessionId);
  const setHeroVisible = useDashboardStore((state) => state.setHeroVisible);
  ...
  }, [currentSessionId, setStoreSessionId]);

  useEffect(() => {
    if (loading || !heroRef.current) return;

    const scrollContainer = heroRef.current.closest("main") || null;
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto pb-12 animate-fade-in text-[var(--text-primary)]">
      ...
      <section ref={heroRef} className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[var(--accent)] to-[var(--accent)]/85 p-8 text-white flex justify-between items-center shadow-md shadow-[var(--accent)]/10">
        <div className="relative z-10 max-w-xl space-y-4">
          ...
          {currentSessionId && (
            <div className="flex gap-4 pt-2">
              <button
                onClick={() => handleResumeSession(router, currentSessionId)}
                aria-label="Resume previous focus session"
                className="bg-white hover:bg-slate-50 text-[var(--accent)] font-bold text-sm px-6 py-3 rounded-xl shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
                Resume Session
              </button>
            </div>
