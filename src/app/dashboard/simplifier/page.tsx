...
export default function SimplifierPage() {
  ...
  const [activeTab, setActiveTab] = useState<"simplified" | "key-points" | "summary" | "assistant" | "quiz">("simplified");
  ...
  useEffect(() => {
    if (activeTab) {
      localStorage.setItem("simplifier_active_tab", activeTab);
    }
  }, [activeTab]);
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      const savedTab = localStorage.getItem("simplifier_active_tab");
      ...
      if (savedSessionId) {
        ...
        try {
          ...
          const matched = sessions.find(s => s.id === savedSessionId);
          if (matched) {
            const chunksRes = await api.getChunks(token, savedSessionId);
  ...
  const handleGenerateQuiz = async (customScope?: string, customDifficulty?: string, customCount?: number) => {
    ...
    setShouldScrollOnTabActive(true);
  ...
  const assistantContainerRef = useRef<HTMLDivElement | null>(null);
  const [shouldScrollOnTabActive, setShouldScrollOnTabActive] = useState(false);
  ...
  // Scroll chat messages container internally
  useEffect(() => {
    if (activeTab === "assistant" && messagesContainerRef.current) {
  ...
  // Smooth scroll quiz container into view after generation
  useEffect(() => {
    if (quizActive && !isQuizLoading && activeTab === "quiz" && shouldScrollOnTabActive) {
      setShouldScrollOnTabActive(false);
  ...
  }, [quizActive, isQuizLoading, activeTab, shouldScrollOnTabActive]);

  // Smooth scroll assistant container into view when opened via Ask AI button
  useEffect(() => {
    if (activeTab === "assistant" && shouldScrollOnTabActive) {
      setShouldScrollOnTabActive(false);
      requestAnimationFrame(() => {
        assistantContainerRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start"
        });
      });
    }
  }, [activeTab, shouldScrollOnTabActive]);
  ...
  const handleSimplify = async () => {
    ...
    try {
      ...
      if (selectedFileObj) {
        ...
        sessionStorage.setItem("session_just_created", uploadRes.session_id);
        ...
        sessionStorage.setItem("session_just_created", activeSession.id);
        router.refresh(); // Invalidate dashboard cache
      }
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto pb-16 animate-fade-in text-[var(--text-primary)]">
      ...
      {/* Restoring Session Loading Indicator */}
      {isRestoring && (
        <div className="flex flex-col items-center justify-center py-16 space-y-4 bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-6 shadow-sm">
          <div className="w-8 h-8 rounded-full border-4 border-[var(--accent)] border-t-transparent animate-spin" />
          <p className="text-xs text-[var(--text-secondary)] font-semibold">Restoring your last workspace session...</p>
        </div>
      )}

      {/* Adaptive Output Canvas */}
      {!isRestoring && hasOutput && chunks.length > 0 && (
        <div className="space-y-6">
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="p-6 md:p-8 min-h-[200px]">
              ...
              {activeTab === "assistant" && (
                <div ref={assistantContainerRef} className="flex flex-col h-[550px] text-left">
                  ...
                  {/* Messages Area */}
                  <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 border border-[var(--border)] rounded-2xl bg-[var(--bg-secondary)]/10 flex flex-col gap-4 mb-4 font-sans">
              ...
              {/* TAB 6: AI Quiz */}
              {activeTab === "quiz" && (
                <div ref={quizContainerRef} className="flex flex-col min-h-[450px] text-left">
                    ...
                    <div className="pt-3 border-t border-[var(--border)] space-y-2.5">
                      ...
                      <div className="flex gap-1.5 w-full">
                        <button
                          ...
                          className="flex-1 bg-[var(--bg-secondary)] hover:bg-[var(--accent-glow)] border border-[var(--border)] hover:border-[var(--accent)] text-[var(--text-secondary)] hover:text-[var(--accent)] font-bold text-[9px] py-1.5 rounded-lg transition-all active:scale-95 text-center whitespace-nowrap cursor-pointer"
                        >
                          💬 Ask AI
                        </button>
                        <button
                        ...
                        <Link href={`/dashboard/focus?session_id=${session?.id}&chunk=${idx + 1}&is_new=true`} className="flex-grow flex-1">
                          <button className="w-full flex items-center justify-center gap-1 bg-[var(--accent)] hover:bg-[var(--accent)]/90 text-white font-bold text-[9px] py-1.5 rounded-lg shadow-sm transition-transform active:scale-95 h-full border-none cursor-pointer">
                            <span>Focus</span>
                            <ArrowRight size={8} />
                          </button>
                        </Link>
