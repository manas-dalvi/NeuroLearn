...
import { api, ProgressStats, Badge, LearningSession, CognitiveProfile, WeeklyXpItem, Quest, StudyActivityResponse, ActivityItem } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import { getLevelProgressInfo, getLevelRankTitle, ACTIVITY_LEVELS, getActivityLevelStyle, DAYS_TO_SHOW } from "@/lib/progression";
...
export default function ProgressPage() {
  ...
  const [hoveredDay, setHoveredDay] = useState<ActivityItem | null>(null);
  ...
  useEffect(() => {
    ...
    const loadProgressData = async () => {
      try {
        const [statsData, badgesData, sessionsData, profileData, weeklyXpData, questsData, activityRes] = await Promise.all([
          ...
          api.getDailyQuests(token).catch(err => {
            console.warn("Failed to load daily quests", err);
            return [];
          }),
          api.getStudyActivity(token).catch(err => {
            console.warn("Failed to load study activity calendar", err);
            return null;
          })
        ]);
        setStats(statsData);
  ...
  // Calculate stats parameters using shared progression helper
  const currentXP = stats?.xp ?? 0;
  const nextLevelXP = stats?.next_level_xp ?? 1000;
  const currentLevel = stats?.level ?? 1;
  const { xpPercent } = getLevelProgressInfo(currentXP, nextLevelXP, currentLevel);
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto pb-12 animate-fade-in text-[var(--text-primary)]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[var(--text-secondary)] font-bold text-xs uppercase tracking-wider mt-0.5">{getLevelRankTitle(currentLevel)}</p>
            </div>
          </div>
        ...
        <section className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-[var(--text-primary)] text-base flex items-center gap-2">
              <Sparkles className="text-[var(--accent)] w-5 h-5 animate-pulse" />
              Daily Quests
            </h3>
            <p className="text-xs font-semibold text-[var(--text-secondary)] mt-0.5">
              Complete these active study objectives today to earn bonus XP.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {quests.map((quest) => {
              const questProgress = quest.target_value > 0 ? (quest.current_value / quest.target_value) * 100 : 0;
              return (
                <div
                  key={quest.id}
                  className={`p-4 rounded-xl border border-[var(--border)] flex flex-col justify-between min-h-[120px] transition-all hover:shadow-md ${quest.completed ? "bg-emerald-500/5 border-emerald-500/20" : "bg-[var(--bg-secondary)]/10"}`}
                >
                  <div className="space-y-1">
                    <div className="flex justify-between items-start gap-2">
                      <span className={`text-[10px] font-extrabold tracking-wide uppercase px-2 py-0.5 rounded-full ${quest.completed ? "bg-emerald-500/10 text-emerald-600" : "bg-[var(--accent-glow)] text-[var(--accent)]"}`}>
                        {quest.name}
                      </span>
                      <span className="text-[10px] font-extrabold text-[var(--accent)] bg-[var(--accent-glow)] px-2 py-0.5 rounded-full">
                        +{quest.xp_reward} XP
                      </span>
                    </div>
                    <p className="text-xs font-bold text-[var(--text-primary)] pt-2 leading-relaxed">{quest.description}</p>
                  </div>
                  ...
                  <div className="space-y-1.5 pt-3">
                    ...
                    <div className="w-full bg-[var(--bg-secondary)] h-2 rounded-full overflow-hidden border border-[var(--border)]/50">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${quest.completed ? "bg-emerald-500" : "bg-[var(--accent)]"}`}
      ...
      {activityData && (
        <section className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border)] shadow-sm space-y-5">
          <div>
            <h3 className="font-bold text-[var(--text-primary)] text-base flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-[var(--text-secondary)] mt-0.5">
              Your learning consistency over the last {DAYS_TO_SHOW} days.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "Study Days", value: activityData.summary.study_days, suffix: ` / ${DAYS_TO_SHOW}`, icon: "📚" },
              { label: "Current Streak", value: activityData.summary.current_streak, suffix: activityData.summary.current_streak === 1 ? " Day" : " Days", icon: "🔥" },
              { label: "Longest Streak", value: activityData.summary.longest_streak, suffix: activityData.summary.longest_streak === 1 ? " Day" : " Days", icon: "🏆" },
              { label: "Total Active Days", value: activityData.summary.total_active_days, suffix: "", icon: "📅" },
            ].map((stat) => (
              <div key={stat.label} className="bg-[var(--bg-secondary)]/20 border border-[var(--border)] rounded-xl p-3.5 space-y-1">
                <div className="text-[10px] font-extrabold text-[var(--text-secondary)] uppercase tracking-wider flex items-center gap-1.5">
                  <span>{stat.icon}</span>
                  <span>{stat.label}</span>
                </div>
                <div className="text-xl font-black text-[var(--text-primary)] tracking-tight">
                  {stat.value}<span className="text-xs font-bold text-[var(--text-secondary)]">{stat.suffix}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="relative">
            <div className="flex flex-wrap gap-[3px]">
              {activityData.calendar.map((day) => (
                <div
                  key={day.date}
                  className="w-[12px] h-[12px] rounded-[3px] cursor-pointer transition-all hover:scale-150 hover:z-10"
                  style={getActivityLevelStyle(day.activity_score)}
            ...
            {hoveredDay && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 pointer-events-none">
                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-lg p-3.5 min-w-[200px] space-y-2">
                  <div className="text-xs font-black text-[var(--text-primary)] border-b border-[var(--border)] pb-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] font-bold">
                    <span className="text-[var(--text-secondary)]">XP Earned</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.xp}</span>
                    <span className="text-[var(--text-secondary)]">Focus Minutes</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.focus_minutes}</span>
                    <span className="text-[var(--text-secondary)]">Chunks Read</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.completed_chunks}</span>
                    <span className="text-[var(--text-secondary)]">Quizzes</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.quizzes_completed}</span>
                    <span className="text-[var(--text-secondary)]">Uploads</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.documents_uploaded}</span>
                    <span className="text-[var(--text-secondary)]">Routines</span>
                    <span className="text-[var(--text-primary)] text-right">{hoveredDay.routines_completed}</span>
                  </div>
                  <div className="flex justify-between items-center pt-1.5 border-t border-[var(--border)]">
                    <span className="text-[10px] font-extrabold text-[var(--text-secondary)] uppercase">Activity Score</span>
                    <span className="text-xs font-black text-[var(--accent)]">{hoveredDay.activity_score}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-2 text-[10px] font-bold text-[var(--text-secondary)]">
            ...
            {ACTIVITY_LEVELS.map((level, idx) => (
              <div key={idx} className="w-[12px] h-[12px] rounded-[3px]" style={level.style} title={level.label} />
            ))}
            <span>More</span>
          </div>
        </section>
