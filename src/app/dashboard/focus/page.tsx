...
function FocusPageContent() {
  ...
  const [sessionsList, setSessionsList] = useState<LearningSession[]>([]);
  ...
  useEffect(() => {
    ...
    const loadChunks = async () => {
      ...
      try {
        ...
        // Find if there is an active/incomplete session in the list
        const incompleteSession = sessions.find(s => s.completed_chunks < s.chunks.length);

        // Check search params first
        const paramSessionId = searchParams.get("session_id");
        const isNewParam = searchParams.get("is_new") === "true";
        const justCreatedId = typeof window !== "undefined" ? sessionStorage.getItem("session_just_created") : null;

        const fetchAndSetupSession = async (activeSessionId: string) => {
          // Clear the temporary flag on initial workspace load
          if (typeof window !== "undefined" && justCreatedId === activeSessionId) {
          ...
          setInitialCompletedCount(completedCount);
          ...
          const nextIdx = res.chunks && res.chunks.length > 0 
            ? Math.min(res.chunks.length, completedCount + 1) 
            : 1;
          setCurrentChunkIdx(nextIdx);
        };

        // 1. Determine navigation intent FIRST
        if (paramSessionId || isNewParam) {
          const targetSessionId = paramSessionId || (incompleteSession ? incompleteSession.id : null);
          if (targetSessionId) {
            setShowContinueCard(false);
            await fetchAndSetupSession(targetSessionId);
          } else {
            setSessionId(null);
            setChunks([]);
          }
          return; // Exit immediately, bypass all resume check logic below
        }

        // 2. Only execute resume detection when entering normally (e.g. from sidebar with no query params)
        if (incompleteSession) {
          setSessionId(incompleteSession.id);
          setSessionTitle(incompleteSession.content_title);
          setShowContinueCard(true);
        } else {
          // No incomplete session found. Fallback to localStorage or first session
          const savedSessionId = localStorage.getItem("current_session_id");
          if (savedSessionId && sessions.some(s => s.id === savedSessionId)) {
            await fetchAndSetupSession(savedSessionId);
          } else if (sessions.length > 0) {
            await fetchAndSetupSession(sessions[0].id);
          }
        }
      } catch (err) {
  ...
  const handleActualResume = async () => {
    if (!token || !sessionId) return;
    setLoading(true);
    try {
      const currentSess = sessionsList.find(s => s.id === sessionId);
      if (currentSess) setSessionTitle(currentSess.content_title);

      const res = await api.getChunks(token, sessionId);
      setChunks(res.chunks || []);
      const completedCount = currentSess ? currentSess.completed_chunks : 0;
      setInitialCompletedCount(completedCount);
      setMaxCompletedCount(completedCount);
      ...
      console.error("Failed to resume session chunks", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (showContinueCard) {
    const matchedSession = sessionsList.find(s => s.id === sessionId);
    const completedCountVal = matchedSession ? matchedSession.completed_chunks : 0;
    const totalChunksCount = matchedSession && matchedSession.chunks ? matchedSession.chunks.length : 0;
    const progressVal = totalChunksCount > 0 ? (completedCountVal / totalChunksCount) * 100 : 0;
    const isResumeTrigger = searchParams.get("session_id") !== null;
    const remainingTime = Math.max(0, (totalChunksCount - completedCountVal) * 2);

    return (
      <div className="max-w-lg mx-auto bg-[var(--bg-card)] border border-[var(--border)] border-l-4 border-l-[var(--accent)] p-8 rounded-2xl shadow-sm text-center space-y-6 animate-fade-in my-12 text-[var(--text-primary)] select-none">
        <div className="space-y-2">
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-[var(--accent)] bg-[var(--accent-glow)] px-3 py-1 rounded-full">
            {isResumeTrigger ? "Continue Your Previous Session" : "Unfinished Session Detected"}
          </span>
          <h2 className="text-xl font-bold text-[var(--text-primary)] flex items-center justify-center gap-2 pt-2">
            📄 {sessionTitle}
          </h2>
        </div>

        {/* Progress details */}
        <div className="space-y-3 bg-[var(--bg-secondary)]/30 p-5 rounded-xl border border-[var(--border)] text-left">
          <div className="flex justify-between text-xs font-bold text-[var(--text-secondary)]">
            <span>Progress</span>
            <span className="text-[var(--accent)]">{completedCountVal} / {totalChunksCount} Chunks Completed</span>
          </div>
          
          {/* Progress Bar */}
          <div className="w-full bg-[var(--bg-secondary)] h-3 rounded-full overflow-hidden border border-[var(--border)]">
            <div 
              className="bg-gradient-to-r from-[var(--accent)] to-[var(--accent)]/80 h-full rounded-full transition-all duration-500" 
              style={{ width: `${progressVal}%` }}
            />
          </div>

          <div className="flex justify-between items-center text-xs font-semibold text-[var(--text-secondary)] pt-1">
            <span>Est. Remaining Time</span>
            <span className="font-bold text-[var(--text-primary)]">{remainingTime} minutes</span>
          </div>
          
          <p className="text-xs text-[var(--text-secondary)] leading-relaxed font-semibold pt-2 border-t border-[var(--border)] mt-2">
            {isResumeTrigger 
              ? "Resume where you left off."
              : "You already have an unfinished learning session. Would you like to resume it or start a new session anyway? Starting a new session will not delete this session's progress."}
          </p>
        </div>

        <div className="flex flex-col gap-3">
          <button 
            onClick={handleActualResume}
            aria-label="Resume previous focus session"
            className="w-full py-3 bg-[var(--accent)] hover:bg-[var(--accent)]/90 text-white rounded-xl text-sm font-bold shadow-sm transition-all hover:scale-[1.01] active:scale-[0.99]"
          >
            Resume Session
          </button>
          ...
          <div className="relative flex py-2 items-center">
            ...
            <span className="flex-shrink mx-4 text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider">or</span>
            <div className="flex-grow border-t border-[var(--border)]"></div>
          </div>
          ...
          <button 
            ...
            className="w-full py-3 bg-transparent border border-[var(--border)] hover:bg-[var(--bg-secondary)]/40 text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-xl text-sm font-bold transition-all"
          >
            Start New Session
          </button>

          <span className="text-[10px] font-semibold text-[var(--text-secondary)] block mt-2">
            Your previous session will remain saved and can be resumed later.
          </span>
        </div>
      </div>
    );
  ...
  const sessionChunkXp = Math.max(0, maxCompletedCount - initialCompletedCount) * 50;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 items-start">
        ...
        <section className="lg:col-span-7 space-y-6 flex flex-col">
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-2 text-left">
            ...
            {/* Profile specific tools (Ruler controls for Dyslexia Mode) */}
                  ...
                  isSimplified ? (
                    <>
                      {activeProfile === "autism" ? (
                        ...
                        <p className="text-[var(--text-primary)] leading-loose">{currentChunk.simplified_text}</p>
                      )}
                    </>
                  ) : (
                    <p className="leading-relaxed text-[var(--text-secondary)]">{currentChunk.original_text}</p>
                  )
                ) : (
        ...
        <section className="lg:col-span-3 space-y-6 flex flex-col">
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-2xl border border-[var(--border)] shadow-sm flex flex-col items-center text-center">
            ...
            <div className="relative w-36 h-36 my-3 flex items-center justify-center select-none">
              <svg viewBox="0 0 144 144" className="w-full h-full -rotate-90 origin-center block">
              ...
              {/* Single centered overlay spanning full parent */}
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-2xl font-black text-[var(--text-primary)] tracking-tight">
                  {formatTime(timerSeconds)}
                </span>
                <span className="text-[8px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mt-0.5">
                  Remaining
                </span>
              </div>
            </div>
