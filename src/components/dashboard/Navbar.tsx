...
export default function Navbar({ onOpenAccessibility }: NavbarProps) {
  ...
  const isHeroVisible = useDashboardStore((state) => state.isHeroVisible);
  const currentSessionId = useDashboardStore((state) => state.currentSessionId);
  ...
  return (
    <header className="h-16 w-full sticky top-0 z-40 bg-[var(--bg-card)] border-b border-[var(--border)] px-6 flex items-center justify-between shadow-sm select-none">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Contextual Session CTA Button */}
        {currentSessionId && !isHeroVisible ? (
          <button 
            onClick={() => handleResumeSession(router, currentSessionId)}
            aria-label="Resume previous focus session"
            className="flex items-center gap-2 px-5 py-2 bg-[var(--accent)] hover:bg-[var(--accent)]/90 text-white rounded-full text-sm font-semibold shadow-sm hover:shadow-md active:scale-[0.98] transition-all"
          >
