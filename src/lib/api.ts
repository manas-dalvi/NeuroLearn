...
export const api = {
  ...
  getDailyQuests: async (token: string): Promise<Quest[]> => {
    return request<Quest[]>("/api/v1/progress/quests", {}, token);
  },

  getStudyActivity: async (token: string): Promise<StudyActivityResponse> => {
    return request<StudyActivityResponse>("/api/v1/progress/activity", {}, token);
  },
