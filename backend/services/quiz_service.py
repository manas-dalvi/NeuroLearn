...
class QuizService:
    ...
    def _parse_json_list(self, text: str) -> List[Dict[str, Any]]:
        """Defensively parses a JSON array or object from LLM string output and returns a list of question dictionaries.
        Supports only:
        1. A JSON array of question dicts: [ {...} ]
        2. A JSON object with a 'questions' key containing a list of question dicts: { "questions": [ {...} ] }
        """
        cleaned = text.strip()
        ...
        try:
            ...
            # Try a regex-based array block extraction
            ...
            if match:
                ...
                # Also try matching a JSON object if array regex failed
                match_obj = re.search(r"\{\s*\".*\"\s*:\s*.*\}", cleaned, re.DOTALL)
                if match_obj:
                    try:
                        parsed = json.loads(match_obj.group(0))
                    except json.JSONDecodeError:
                        raise ValueError("LLM response did not contain a valid JSON object of questions.")
                else:
                    raise ValueError("LLM response did not contain a valid JSON format.")

        # Support only the normalized formats:
        if isinstance(parsed, list):
            # Format 1: A JSON array of question dicts
            if all(isinstance(item, dict) for item in parsed):
                return parsed
            raise ValueError("LLM returned an unexpected structure: Expected a list of question dictionaries.")

        if isinstance(parsed, dict):
            # Format 2: A JSON object with a 'questions' key containing a list of question dicts
            if "questions" in parsed and isinstance(parsed["questions"], list):
                questions = parsed["questions"]
                if all(isinstance(item, dict) for item in questions):
                    return questions
            raise ValueError(
                "LLM returned an unexpected structure: Expected a JSON object with a 'questions' list."
            )

        raise ValueError(
            "LLM returned an unexpected structure: Could not extract a valid list of question dictionaries."
        )
    ...
    async def generate_quiz(
        ...
        system_prompt = (
            ...
            "{profile_rules}\n\n"
            "Respond ONLY with a JSON object containing a 'questions' array. Ensure correct JSON syntax. "
            "Structure the JSON response EXACTLY like this:\n"
            "{{\n"
            "  \"questions\": [\n"
            "    {{\n"
            "      \"id\": \"q1\",\n"
        ...
        logger.info("PARSING VERIFICATION:")
        logger.info("- raw_qs type: %s", type(raw_qs))
        logger.info("- raw_qs len: %s", len(raw_qs))
        logger.info("- is raw_qs a List[Dict]? %s", isinstance(raw_qs, list) and all(isinstance(x, dict) for x in raw_qs))

        # Normalize keys and structure
        normalized = []
        for i, q in enumerate(raw_qs):
            logger.info("PROCESSING QUESTION %d:", i)
            logger.info("- q type: %s", type(q))
            logger.info("- q value: %s", q)
        ...
        logger.info("- normalized is a List[Dict]? %s", isinstance(normalized, list) and all(isinstance(x, dict) for x in normalized))
        try:
            dumped = json.dumps(normalized)
            logger.info("- json.dumps(normalized) succeeded. Length: %d", len(dumped))
        except Exception as e:
            logger.error("- json.dumps(normalized) failed: %s", e, exc_info=True)
            raise

        logger.info("DATABASE SAVE VERIFICATION:")
        new_attempt = QuizAttempt(
            session_id=session_id,
            chunk_id=chunk_id,
            difficulty=difficulty,
        ...
        try:
            logger.info("- Attempting db.refresh(new_attempt)...")
            await db.refresh(new_attempt)
            logger.info("- db.refresh() succeeded. new_attempt.id: %s", new_attempt.id)
        except Exception as e:
            logger.error("- db.refresh() failed: %s", e, exc_info=True)
            raise

        logger.info("RESPONSE CREATION VERIFICATION:")
        try:
            sanitized = [
                QuizQuestionSchema(
                    id=q["id"],
            ...
            response = QuizGenerationResponse(quiz_attempt_id=new_attempt.id, questions=sanitized)
            logger.info("- QuizGenerationResponse created successfully: %s", response)
            return response
        except Exception as e:
            logger.error("- QuizGenerationResponse creation failed: %s", e, exc_info=True)
            raise
    ...
    async def submit_quiz(
        ...
        await ProgressService.update_quest_progress(db, user_id, "complete_quizzes", 1)
